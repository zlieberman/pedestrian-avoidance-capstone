...
def mount_smb(username, password, dataset_folder, smb_json_path):
    ...
    image_folder = dataset_folder + '/images/train/*.jpg'
    video_name = 'video.mp4'
    images = glob.glob(image_folder)
    print(images[0])
    frame = cv2.imread(images[1000])
    h, w, l = frame.shape

    video = cv2.VideoWriter(video_name, cv2.VideoWriter_fourcc(*'mp4v'), 15, (w, h))

    for img in sorted(images)[800:1100]:
