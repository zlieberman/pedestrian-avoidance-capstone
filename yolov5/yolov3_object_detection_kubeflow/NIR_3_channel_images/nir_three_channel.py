...
if __name__ == "__main__":
    ...
    train_files = [Path(p).name for p in glob.glob('/app/labels/labels_05_13_20_multispectral/*.txt')]
    val_files = [Path(p).name for p in glob.glob('/app/labels/labels_06_03_20_multispectral/*.txt')]
    print('training files: {}'.format(len(train_files)))
    print('validation files: {}'.format(len(val_files)))

    smb_path = '/ms_2_images/NIR1/frames/'
    train_dataset_folder = '/app/labels_05_13_20_NIR'
    val_dataset_folder = '/app/labels_06_03_20_NIR'
