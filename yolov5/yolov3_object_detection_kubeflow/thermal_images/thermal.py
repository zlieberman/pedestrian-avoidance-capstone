...
if __name__ == "__main__":
    ...
    files = glob.glob('/app/labels_05_13_20_thermal/labels_05_13_20_thermal/*.txt')
    print('files: {}'.format(len(files)))
    username = ARGS.username
    password = ARGS.password
    mount_smb(username, password, '/app/labels_05_13_20_thermal', [Path(f).name for f in files[:6700]], [Path(f).name for f in files[6700:]])
