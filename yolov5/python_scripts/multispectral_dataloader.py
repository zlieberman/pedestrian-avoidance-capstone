	...
	for di in dirs:
		...
		for file in files:
			...
			if file.replace('.tiff', '.txt') in train_imgs:
				train_count += 1
				write_to_YOLO(dataset_folder, labels_folder,
...
if __name__ == "__main__":
	...
	mount_smb(username, password, smb_path, data_folder, labels_folder, [Path(f).name for f in files[:1600]],
			  [Path(f).name for f in files[1600:]])